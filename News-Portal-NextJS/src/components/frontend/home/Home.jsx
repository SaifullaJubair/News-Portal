...
const Home = () => {
  ...
  const videoSection = special_news?.data?.find(
    (item) => item?.categoryDetails?.special_category_serial == "2"
  );
  const photoSection = special_news?.data?.find(
    (item) => item?.categoryDetails?.special_category_serial == "1"
  );
    ...
    <div className="tracking-wide  ">
      <div className="space-y-4 mb-6 ">
        ...
        <>
          ...
          {newsLoading ? null : (
            <GridCategoryNews newsData2ndCol={news?.data?.[0]} />
          )}
          {!newsLoading && !adsLoading && ad4 && (
            <div className="flex item-center justify-center ">
              <a target="_blank" rel="noreferrer" href={`${ad4?.ads_link}`}>
                <img src={ad4?.ads_image} alt="" className="h-20 sm:h-[90px]" />
              </a>
            </div>
          )}
          {/* OnlineJorip end*/}
          ...
          {!newsLoading && !adsLoading && ad6 && (
            <div className="flex item-center justify-center ">
              <a target="_blank" rel="noreferrer" href={`${ad6?.ads_link}`}>
                <img src={ad6?.ads_image} alt="" className="h-20 sm:h-[90px]" />
              </a>
            </div>
          )}
          ...
          {!newsLoading && !adsLoading && ad7 && (
            <div className="flex item-center justify-center ">
              <a target="_blank" rel="noreferrer" href={`${ad7?.ads_link}`}>
                <img src={ad7?.ads_image} alt="" className="h-20 sm:h-[90px]" />
              </a>
            </div>
          )}
          ...
          {newsLoading ? null : (
            <SectionEleven
              newsData={news?.data?.[7]?.news}
              newsLoading={newsLoading}
              categoryData={news?.data?.[7]}
              adsData={ad16}
            />
          )}
          ...
          {/* Business */}
          {newsLoading ? null : (
            <SectionThree newsData={news?.data?.[8]} adsData={ad17} />
          )}

          {/* Education */}
          {newsLoading ? null : <SectionSeven newsData={news?.data?.[9]} />}
          {!newsLoading && !adsLoading && ad3 && (
            <div className="flex item-center justify-center ">
              <a target="_blank" rel="noreferrer" href={`${ad3?.ads_link}`}>
                <img src={ad3?.ads_image} alt="" className="h-20 sm:h-[90px]" />
      ...
      {specialNewsLoading ? null : (
        <div className="mb-4">
          <PhotoGallery special_news={photoSection} adsData={ad18} />
        </div>
      )}
    </div>
  );
