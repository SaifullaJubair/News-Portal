...
const LogoVidio = ({ refetch, initialData, user }) => {
  ...
  const handleDataPost = async (data) => {
    ...
    if (data?.cover_photo?.[0]) {
      const cover_photoUpload = await ImageUploader(data?.cover_photo?.[0]);
      cover_photo = cover_photoUpload[0];
    }
    const sendData = {
      logo: logo || initialData?.logo,
      favicon: favicon || initialData?.favicon,
      cover_photo: cover_photo || initialData?.cover_photo,
    ...
    <>
      <form onSubmit={handleSubmit(handleDataPost)}>
        <div className="grid gap-6 grid-cols-2">
          ...
          <div>
            <label className="font-semibold" htmlFor="cover_photo">
              Reporter Cover Photo<span className="text-red-500"> if need</span>{" "}
            </label>
            <input
