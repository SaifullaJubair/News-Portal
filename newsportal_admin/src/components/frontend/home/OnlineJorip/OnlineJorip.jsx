...
import { FacebookIcon, FacebookMessengerIcon, FacebookMessengerShareButton, FacebookShareButton, LinkedinIcon, LinkedinShareButton, TwitterIcon, TwitterShareButton, WhatsappIcon, WhatsappShareButton } from "react-share";
import { toast } from "react-toastify";

const OnlineJorip = () => {
  const [index, setIndex] = useState([]);

  const { data: joripData = [], isLoading: joripLoading, refetch } = useQuery({
  ...
  const handleVote = async (option, jorip, click, questionAnsId) => {
    ...
    const sendData = {
      _id: jorip?._id,
      online_jorip_all_question: jorip?.online_jorip_all_question?.map((item) => {
            ...
            <div key={jorip?._id} className="px-2 group mt-2 border rounded-md">
              ...
              <div className="space-y-4 mb-2">
                {
                  index.includes(i) ?
                    jorip?.online_jorip_all_question?.map((question) => {
                      ...
                      const totalClicks = jorip?.online_jorip_all_question?.reduce(
                      ...
                      const percentage = totalClicks
                        ? ((question?.online_jorip_question_click || 0) / totalClicks) * 100
                        : 0;

                      return (
                        <div key={question?._id} className="relative w-full h-6 bg-gray-200 rounded-lg">
                          {/* Progress bar */}
                          <div
                          ...
                          <span className="absolute inset-0 flex justify-between px-2 text-white text-sm items-center">
                            {question?.online_jorip_question} - {question?.online_jorip_question_click || 0}
                    ...
                    jorip?.online_jorip_all_question?.map((question) => (
                      <button
                        key={question?._id}
                        onClick={() => handleVote(i, jorip, question?.online_jorip_question_click, question?._id)}
              ...
              <div className="flex gap-3 my-2 mr-2 items-center justify-center flex-wrap">
                ...
                <button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(`https://daily-our-bangladesh-j9pfe.ondigitalocean.app/online_jorip?jorip_id=${jorip?._id}`)
                      .then(() => {
