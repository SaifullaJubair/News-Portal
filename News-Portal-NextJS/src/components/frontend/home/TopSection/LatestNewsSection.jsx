...
const LatestNewsSection = ({ latestNews, adsData }) => {
  return (
    <div className="">
      <div className="mb-4">
        {adsData && (
          <Link target="_blank" rel="noreferrer" href={`${adsData?.ads_link}`}>
            <img
        ...
        <div className="max-h-[700px] border rounded overflow-y-auto scrollbar-thin pr-1 scrollbar-track-gray-50 scrollbar-thumb-primaryMediumLightColor/50 ">
          ...
          <div className="mx-4 ">
            {latestNews?.data?.map((news, index) => (
              ...
              >
                <h1
                  className="font-extrabold mb-2 hover:text-pHoverTextColor duration-150  md:max-w-lg w-full mt-5 text-pTextColor group-hover:text-pHoverTextColor"
                  style={{
                    overflow: "hidden",
                    display: "-webkit-box",
                    WebkitLineClamp: "1",
                ...
                {news?.createdAt && (
                  <p className="text-sm flex gap-2 items-center text-gray-700 my-1">
