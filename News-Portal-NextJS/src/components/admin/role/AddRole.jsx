...
const AddRole = ({ refetch, setIsAddModalOpen, token }) => {
  const {
    register,
    reset,
    handleSubmit,
      ...
      <div className="relative overflow-hidden text-left bg-white  shadow-xl xl:w-[1150px] lg:w-[950px] md:w-[760px] sm:w-[600px]  w-[550px] p-6 max-h-[100vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button className="btn bg-white hover:bg-white border p-1">
            <RxCross1 onClick={() => setIsAddModalOpen(false)} size={25} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(handleDataPost)}>
          ...
          {/* Permissions */}
          <div className="mt-4">
            <h4 className="text-base font-semibold  mb-2">Permissions</h4>
            <div className="">
              {permissionsData.map(renderPermissionSection)}
            </div>
          </div>
