...
const AddJorip = ({ refetch, setIsJoripModalOpen, token }) => {
  ...
  const handleDataPost = async (data) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      } else if (key == "social_image") {
        formData.append(key, data?.social_image[0]);
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/online_jorip?role_type=online_jorip_create`,
        {
          method: "POST",
          headers: {
      ...
      if (result?.statusCode == 200 && result?.success == true) {
        toast.success(
          result?.message
            ? result?.message
            : "Online Jorip created successfully",
          {
            autoClose: 1000,
          }
        );
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        ...
        >
          ...
          <div>
            ...
            <label className="ml-1 text-sm font-semibold py-1 text-gray-700">
              Online jorip Image
            </label>
            <input
            ...
            {errors.online_jorip_image && (
              <p className="text-red-600">
                {errors.online_jorip_image?.message}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.online_jorip_status && (
              <p className="text-red-600">
                {errors.online_jorip_status?.message}
              </p>
            )}
          </div>
          <div>
            ...
            {errors.online_jorip_title && (
              <p className="text-red-600">
                {errors.online_jorip_title?.message}
              </p>
            )}
          </div>
