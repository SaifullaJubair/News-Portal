"use client";
import { useGetJoripData } from "@/api/useGetJoripData";
...
const GridCategoryNews = ({ newsData2ndCol }) => {
  const [index, setIndex] = useState([]);

  const { data: joripData, isLoading, refetch } = useGetJoripData();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 sm:gap-6 tracking-wide">
            ...
            joripData?.map((jorip, i) => (
              <div key={i} className="">
                ...
                <div className="flex items-center  gap-2 sm:gap-4 mt-1 mb-5  group">
                  ...
                  <Link href={`/online_jorip`} className="relative ">
        ...
        <div className="col-span-2">
          <div className="flex items-center  gap-2  sm:gap-4 mt-1 mb-5 group">
            <div className="flex-grow border-y h-[5px] border-gray-500 group-hover:border-pHoverTextColor"></div>
            <Link
              href={`/${newsData2ndCol?.categoryDetails?.category_slug}`}
              className="relative "
            >
            ...
            <div className="flex flex-col h-full justify-between border rounded">
              <div className="flex-1">
                {newsData2ndCol?.news?.slice(0, 1).map((item) => (
                  <Link
                    href={`/${newsData2ndCol?.categoryDetails?.category_slug}/${item?._id}`}
                    key={item?._id}
