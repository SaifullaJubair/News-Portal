    ...
    <div className="max-w-[1400px] w-[95%] mx-auto">
      <div>
        ...
        <div className="flex items-center justify-between py-2 border-b-4 border-gray-800">
          <Link
            to={`/${categoryData?.categoryDetails?.category_slug}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 py-6 tracking-wide">
          ...
          <div className="flex flex-col gap-1 px-2">
            {newsData?.slice(0, 4)?.map((item, index) => (
              <Link
                to={`${categoryData?.categoryDetails?.category_slug}/${item?._id}`}
                className="cursor-pointer hover:opacity-90 group"
                key={index}
              >
                <div className="mt-2 space-y-2">
                  <p
                    className="font-semibold text-pHeadingTextColor text-[16px] group-hover:text-pHoverTextColor"
                    style={{
                      overflow: "hidden",
                      display: "-webkit-box",
                      WebkitLineClamp: "2",
          ...
          >
            ...
            <div className="p-2 space-y-2">
              <p
                className="font-semibold text-pHeadingTextColor text-[16px] lg:text-[20px] group-hover:text-pHoverTextColor"
                style={{
                  overflow: "hidden",
                  display: "-webkit-box",
                  WebkitLineClamp: "2",
