...
const SubCategory = () => {
  ...
  const {
    ...
    queryKey: [
      `/api/v1/sub_category/dashboard?page=${page}&limit=${rows}&searchTerm=${searchTerm}&role_type=sub_category_show`,
    ],
    queryFn: async () => {
      const res = await fetch(
        `${BASE_URL}/sub_category/dashboard?page=${page}&limit=${rows}&searchTerm=${searchTerm}&role_type=sub_category_show`,
        {
          headers: {
        ...
        <>
          ...
          <div className="my-10 bg-white border border-gray-200 rounded-xl">
            <div className="flex items-center justify-between p-4 gap-2">
              ...
              {user?.role_id?.sub_category_create && (
                <button
          ...
          <SubCategoryTable
            subCategoryTypes={subCategoryTypes?.data}
            refetch={refetch}
            user={user}
            token={token}
          />

          {subCategoryTypes?.totalData > 10 && (
            <Pagination
              rows={rows}
              page={page}
              setPage={setPage}
